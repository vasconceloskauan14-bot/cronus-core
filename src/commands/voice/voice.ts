...
export const call: LocalCommandCall = async () => {
  ...
  const replyNote =
    ' Claude will also read completed text replies aloud by default. Set `voiceReplyEnabled` to false in settings to mute spoken replies.'
  return {
    type: 'text' as const,
    value: `Voice mode enabled. Hold ${key} to record.${langNote}${replyNote}`,
  }
}
